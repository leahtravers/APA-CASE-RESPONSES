#!/usr/bin/env python3
"""V55 contract-version-correct isolated agent adapter.
...
CONTRACT_VERSION = os.environ.get("CONTRACT_VERSION", "RI-CONTRACT-V55").strip() or "RI-CONTRACT-V55"
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v55_session_recovery")
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v55_contract_isolated",
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v55_contract_isolated_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v55_contract_isolated_adjudication",
            state_path,
            trace_path,
            "verified",
        )
