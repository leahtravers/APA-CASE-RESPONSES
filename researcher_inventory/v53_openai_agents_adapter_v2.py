#!/usr/bin/env python3
"""V53 harness successor: contract-version-correct isolated agent adapter.

This corrects a harness defect in the predecessor V53 adapter path. The saved agent
is bootstrapped from the durable contract named by RI_CONTRACT_FILE; session prompts
must not tell that agent to apply a different predecessor contract. Gold archetypes,
evaluator findings, prior scored outputs, and sealed holdout content are never sent
to the worker.
"""
...
CONTRACT_VERSION = os.environ.get("CONTRACT_VERSION", "RI-CONTRACT-V53").strip() or "RI-CONTRACT-V53"
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v53_session_recovery")
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v53_contract_isolated_v2",
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v53_contract_isolated_v2_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v53_contract_isolated_v2_adjudication",
            state_path,
            trace_path,
            "verified",
        )
