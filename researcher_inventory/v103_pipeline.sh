...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v103_session_recovery || cp -R researcher_inventory/runtime/v103_session_recovery "$HISTORY_DIR/v103_session_recovery"
...
print(json.dumps({
  ...
  'harness_correction':'V66_HARNESS_CORRECTION_2026-09-17 + V82_ATTEMPT_HISTORY_DURABILITY + V88A_EDITOR_SHORTHAND_ALIGNMENT_2026-09-18 + V92_SAME_SESSION_TRANSIENT_RECOVERY',
  'semantic_successor':'V103',
  'semantic_correction':'COMPREHENSIVE_SOURCE_REIFIED_RECOVERY_PLUS_CANONICAL_CLASS_NATIVE_PROJECTION'
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m unittest researcher_inventory.tests.test_inventory_apparatus researcher_inventory.tests.test_v64_compound_order_harness researcher_inventory.tests.test_v66_harness_correction researcher_inventory.tests.test_v88a_harness_alignment -v
python -m py_compile researcher_inventory/v103_task_rules.py researcher_inventory/v103_openai_agents_adapter.py researcher_inventory/v103_harness_calibration_runner.py researcher_inventory/v103_harness_holdout_runner.py
...
echo '=== V103 DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v103_harness_calibration_runner

echo '=== V103 REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v103_harness_calibration_runner

echo '=== V103 REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v103_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V103 comprehensive source-reified recovery plus canonical class-native projection passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF

# First and only calibrated-lineage sealed holdout attempt, reachable only after all paired archetype batches pass.
python -m researcher_inventory.v103_harness_holdout_runner
...
# session_bootstrap always POSTs a brand-new saved agent. It receives only the finalized V103 durable contract.
python researcher_inventory/session_bootstrap.py
python -m researcher_inventory.v103_harness_holdout_runner

cat > researcher_inventory/status/CLEAN_ROOM_CERTIFIED.md <<'EOF'
# Researcher Inventory Apparatus Clean-Room Certification
V103 passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one finalized contract. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V103 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
