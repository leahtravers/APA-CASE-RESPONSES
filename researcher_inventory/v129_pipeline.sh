...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v129_session_recovery || cp -R researcher_inventory/runtime/v129_session_recovery "$HISTORY_DIR/v129_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V129_NATURAL_EDITORIAL_GRAIN_PRIMARY_RESEARCH_UNITS',
...
if (r/'calibrated_lineage_holdout_result.json').exists():
  out['calibrated_lineage_holdout_pass']=bool(json.loads((r/'calibrated_lineage_holdout_result.json').read_text()).get('pass'))
if (r/'retired_calibration_agent_id.txt').exists() and (r/'holdout_result.json').exists():
  out['latest_holdout_pass']=bool(json.loads((r/'holdout_result.json').read_text()).get('pass'))
print(json.dumps(out,sort_keys=True,indent=2))
PY
  git add "$HISTORY_DIR" researcher_inventory/status 2>/dev/null || true
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V129 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m unittest researcher_inventory.tests.test_inventory_apparatus researcher_inventory.tests.test_v64_compound_order_harness researcher_inventory.tests.test_v66_harness_correction researcher_inventory.tests.test_v88a_harness_alignment researcher_inventory.tests.test_v126a_prompt_harness -v
python -m py_compile researcher_inventory/v126a_task_rules.py researcher_inventory/v126a_harness_prompt_correction.py researcher_inventory/v126a_harness_calibration_runner.py researcher_inventory/v126a_harness_holdout_runner.py researcher_inventory/v129_openai_agents_adapter.py
...
if attempts:
  ...
  Path('researcher_inventory/status/HOLDOUT_CONSUMED_HOLD.md').write_text(
    '# Researcher Inventory Sealed Holdout Hold\n\n'
    f'The sealed holdout has already been attempted in workflow history `{run_id}`. Automatic reuse is prohibited. Preserve the prior result and do not reopen or expose the sealed source for calibration.\n'
  )
...
echo '=== V129 CONTRACT / V126A CONTRACT-SUBORDINATE HARNESS DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V129 CONTRACT / V126A HARNESS REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V129 CONTRACT / V126A HARNESS REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V129 durable contract with the retained V126A contract-subordinate runtime prompt harness passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF

# First and only calibrated-lineage sealed holdout attempt, reachable only after all paired archetype batches pass.
python -m researcher_inventory.v126a_harness_holdout_runner
...
# session_bootstrap always POSTs a brand-new saved agent. It receives only finalized V129 durable instructions from RI_CONTRACT_FILE.
python researcher_inventory/session_bootstrap.py
python -m researcher_inventory.v126a_harness_holdout_runner

cat > researcher_inventory/status/CLEAN_ROOM_CERTIFIED.md <<'EOF'
# Researcher Inventory Apparatus Clean-Room Certification
The finalized V129 durable contract, executed through the retained V126A contract-subordinate runtime prompt harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V129 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
