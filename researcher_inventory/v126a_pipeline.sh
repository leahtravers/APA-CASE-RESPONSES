...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v126_session_recovery || cp -R researcher_inventory/runtime/v126_session_recovery "$HISTORY_DIR/v126_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V126_UNCHANGED',
  'harness_successor':'V126A_PROMPT_COMPOSITION_CORRECTION',
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V126A run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m py_compile researcher_inventory/v126a_task_rules.py researcher_inventory/v126a_harness_prompt_correction.py researcher_inventory/v126a_harness_calibration_runner.py researcher_inventory/v126a_harness_holdout_runner.py researcher_inventory/v126_openai_agents_adapter.py
...
echo '=== V126A HARNESS-CORRECTED / V126 CONTRACT DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V126A HARNESS-CORRECTED / V126 CONTRACT REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V126A HARNESS-CORRECTED / V126 CONTRACT REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V126 durable contract with the V126A contract-subordinate runtime prompt harness passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF
...
# session_bootstrap always POSTs a brand-new saved agent. It receives only finalized V126 durable instructions.
python researcher_inventory/session_bootstrap.py
python -m researcher_inventory.v126a_harness_holdout_runner

cat > researcher_inventory/status/CLEAN_ROOM_CERTIFIED.md <<'EOF'
# Researcher Inventory Apparatus Clean-Room Certification
The finalized V126 durable contract, executed through the V126A contract-subordinate runtime prompt harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V126 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
