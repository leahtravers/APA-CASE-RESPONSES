#!/usr/bin/env python3
"""V40 harness-isolated adapter.

The durable V40 contract is the sole semantic selection/grain authority. The
...
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v40_contract_isolated_session_recovery")
...
def _attention_for(bounded: dict) -> str:
    ...
    if correction:
        correction_note = (
            ...
            + correction
            + " Rebuild the complete requested result under the installed V40 contract; "
    ...
    if bounded.get("task") == "researcher_inventory_extract_one_class":
        return (
            "UNIT PASS. Apply the installed durable V40 contract exactly to the complete source "
            "for the requested class. Reconstruct the source binding/frame skeleton first, then "
            "retain only units that fill distinct inventory-bearing roles in that skeleton at the "
            "contract's natural complete class grain. Do not use V38 whole-binding deletion and do "
            "not use V39 locally typable positivity as sufficient admission. Follow response_schema "
    ...
    if bounded.get("task") == "researcher_inventory_build_lightweight_compounds":
        return (
            "COMPOUND PASS. Apply the installed durable V40 contract exactly to the complete source "
            "and supplied frozen final units. Reconstruct the same source binding skeleton used for "
            "unit admission and serialize each representable source-presented binding once at its "
            "natural source grain. Do not create, suppress, or repair units. Follow response_schema "
            "exactly. Do not infer an expected count or hidden archetype."
            + correction_note
        )
    return "Apply the installed durable V40 contract exactly to this bounded request." + correction_note
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v40_contract_isolated",
            detail="prior local state preserved in trace; new request starts fresh",
        )
        state_path.unlink()

    initial_prompt = (
        "Execute only this bounded request under your installed durable V40 contract. "
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v40_contract_isolated_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verify_prompt = (
            "Perform a separate adjudication in a new session under the SAME installed durable V40 "
            "contract. Re-read the complete source and independently rebuild the requested result "
            "before comparing with the provisional output. Reconstruct the source binding/frame "
            "skeleton first and apply binding-role admission at natural complete class grain. The V40 "
            "durable contract remains the sole semantic selection/grain authority. Return the complete "
            "replacement JSON, not a critique. Do not infer or reconstruct an archetype, expected "
            "count, evaluator preference, scored prior output, case-specific gold correction, sealed "
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v40_contract_isolated_adjudication",
            state_path,
            trace_path,
            "verified",
        )
