...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v138_session_recovery || cp -R researcher_inventory/runtime/v138_session_recovery "$HISTORY_DIR/v138_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V138_REPRESENTATION_LEDGER_PROJECTION_COMPLETE_PREDICATE_KERNEL',
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V138 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m py_compile researcher_inventory/v126a_task_rules.py researcher_inventory/v126a_harness_prompt_correction.py researcher_inventory/v126a_harness_calibration_runner.py researcher_inventory/v126a_harness_holdout_runner.py researcher_inventory/v138_openai_agents_adapter.py
...
The finalized V138 durable representation-ledger contract, executed through the retained V126A contract-subordinate runtime prompt harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V138 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
