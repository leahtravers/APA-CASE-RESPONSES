#!/usr/bin/env python3
"""V149A whole-source class-local inventory-coordinate / complete-atom harness.

RI-CONTRACT-V149 remains the sole durable semantic authority. This successor retains
the proven single-turn whole-source topology while changing only the operation selector
so it follows V149's class-local coordinate and complete-source-atom order. No
gold/evaluator/holdout material is included.
"""
...
from researcher_inventory import v148a_class_complete_harness as v148a
from researcher_inventory.v126a_task_rules import V126A_BASE_RULES
from researcher_inventory.v149a_task_rules import V149A_INTEGRATED_RULE


class V149AInventoryCoordinateApparatus(v148a.V148AClassCompleteInventoryApparatus):
    """Execute V149 semantics in one coherent whole-source model turn."""

    def _extract_integrated(self, source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        payload = {
            "task": "researcher_inventory_build_class_local_coordinate_inventory",
            "rules": V126A_BASE_RULES + "\n" + V149A_INTEGRATED_RULE,
            "source": source,
            "response_schema": {
        ...
        raise apparatus.ApparatusError(f"integrated V149A extraction failed after retries: {last}")
    ...
    def run(self, source: str, source_case_ref: str | None = None, researcher_interest: str | None = None, created_by_ref: str = "RI-WORKER") -> dict[str, Any]:
        ...
        result["validation"]["notes"] = ["V149A one-turn whole-source class-local inventory-coordinate / complete-source-atom semantic pass plus mechanical validation passed"]
        return result
