#!/usr/bin/env python3
"""V36 harness-isolated adapter.

This successor exists to test the installed durable V36 contract without a second
semantic policy layer from inventory_apparatus.py. The apparatus still owns
mechanical schema/source validation and sends task data, but its legacy `rules`
and `class_rule` helper prose are not forwarded to the semantic worker.
...
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v36_contract_isolated_session_recovery")
...
def _attention_for(bounded: dict) -> str:
    ...
    if correction:
        correction_note = (
            ...
            + correction
            + " Rebuild the complete requested result under the installed V36 contract; "
    ...
    if bounded.get("task") == "researcher_inventory_extract_one_class":
        return (
            "UNIT PASS. Apply the installed durable V36 contract exactly to the complete source "
            "for the requested class. Read the whole source before returning the complete class "
            "result. Follow response_schema exactly. Do not infer an expected count or hidden archetype."
            + correction_note
        )
    if bounded.get("task") == "researcher_inventory_build_lightweight_compounds":
        return (
            "COMPOUND PASS. Apply the installed durable V36 contract exactly to the complete source "
            "and the supplied frozen final units. Follow response_schema exactly and use only supplied "
            "unit_ref values. Do not infer an expected count or hidden archetype."
            + correction_note
        )
    return "Apply the installed durable V36 contract exactly to this bounded request." + correction_note
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v36_contract_isolated",
            detail="prior local state preserved in trace; new request starts fresh",
        )
        state_path.unlink()

    initial_prompt = (
        "Execute only this bounded request under your installed durable V36 contract. "
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v36_contract_isolated_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verify_prompt = (
            "Perform a separate adjudication in a new session under the SAME installed durable V36 "
            "contract. Re-read the complete source and independently rebuild the requested result "
            "before comparing with the provisional output. The V36 durable contract remains the sole "
            "semantic selection/grain authority. Return the complete replacement JSON, not a critique. "
            "Do not infer or reconstruct an archetype, expected count, evaluator preference, scored prior "
            "output, case-specific gold correction, sealed holdout source, or holdout output.\n\n"
            "TASK ATTENTION:\n" + attention + "\n\n"
            "REQUEST JSON:\n" + json.dumps(bounded, ensure_ascii=False) + "\n\n"
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v36_contract_isolated_adjudication",
            state_path,
            trace_path,
            "verified",
        )
