#!/usr/bin/env python3
"""V150A whole-source promoted-coordinate / local-nonredundancy harness.

RI-CONTRACT-V150 remains the sole durable semantic authority. This successor retains
the proven single-turn whole-source topology while changing only the subordinate
operation selector so it follows V150's promoted-coordinate and primary-role order.
No gold/evaluator/holdout material is included.
"""
...
class V150APromotedCoordinateApparatus(v149a.V149AInventoryCoordinateApparatus):
    """Execute V150 semantics in one coherent whole-source model turn."""

    def _extract_integrated(self, source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        payload = {
            "task": "researcher_inventory_build_promoted_coordinate_inventory",
            "rules": V126A_BASE_RULES + "\n" + V150A_INTEGRATED_RULE,
            "source": source,
            "response_schema": {
        ...
        raise apparatus.ApparatusError(f"integrated V150A extraction failed after retries: {last}")
    ...
    def run(self, source: str, source_case_ref: str | None = None, researcher_interest: str | None = None, created_by_ref: str = "RI-WORKER") -> dict[str, Any]:
        ...
        result["validation"]["notes"] = ["V150A one-turn whole-source promoted-coordinate / local-nonredundancy semantic pass plus mechanical validation passed"]
        return result
