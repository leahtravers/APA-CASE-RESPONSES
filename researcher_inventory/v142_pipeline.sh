...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v142_session_recovery || cp -R researcher_inventory/runtime/v142_session_recovery "$HISTORY_DIR/v142_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V142_SOURCE_ATOMIC_LOCAL_BINDING_RESOLUTION',
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V142 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m py_compile researcher_inventory/v126a_task_rules.py researcher_inventory/v126a_harness_prompt_correction.py researcher_inventory/v126a_harness_calibration_runner.py researcher_inventory/v126a_harness_holdout_runner.py researcher_inventory/v142_openai_agents_adapter.py
...
The finalized V142 durable source-atomic local-binding resolution contract, executed through the retained V126A contract-subordinate runtime prompt harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V142 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
