#!/usr/bin/env python3
"""V37 harness-isolated adapter.

The durable V37 contract is the sole semantic selection/grain authority. The
apparatus still supplies bounded task/mechanical data and owns mechanical
schema/source validation, but legacy helper semantic prose is not forwarded.
...
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v37_contract_isolated_session_recovery")
...
def _attention_for(bounded: dict) -> str:
    ...
    if correction:
        correction_note = (
            ...
            + correction
            + " Rebuild the complete requested result under the installed V37 contract; "
    ...
    if bounded.get("task") == "researcher_inventory_extract_one_class":
        return (
            "UNIT PASS. Apply the installed durable V37 contract exactly to the complete source "
            "for the requested class. Read the whole source, reconstruct the source-binding map, "
            "and return the complete class result at the contract's atomic class-native grain. "
    ...
    if bounded.get("task") == "researcher_inventory_build_lightweight_compounds":
        return (
            "COMPOUND PASS. Apply the installed durable V37 contract exactly to the complete source "
            "and supplied frozen final units. Replay the source-binding map using only supplied "
            "unit_ref values. Follow response_schema exactly. Do not infer an expected count or hidden archetype."
            + correction_note
        )
    return "Apply the installed durable V37 contract exactly to this bounded request." + correction_note
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v37_contract_isolated",
            detail="prior local state preserved in trace; new request starts fresh",
        )
        state_path.unlink()

    initial_prompt = (
        "Execute only this bounded request under your installed durable V37 contract. "
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v37_contract_isolated_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verify_prompt = (
            "Perform a separate adjudication in a new session under the SAME installed durable V37 "
            "contract. Re-read the complete source and independently rebuild the requested result "
            "before comparing with the provisional output. The V37 durable contract remains the sole "
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v37_contract_isolated_adjudication",
            state_path,
            trace_path,
            "verified",
        )
