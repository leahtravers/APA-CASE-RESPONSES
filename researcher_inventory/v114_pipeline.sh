...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v114_session_recovery || cp -R researcher_inventory/runtime/v114_session_recovery "$HISTORY_DIR/v114_session_recovery"
...
print(json.dumps({
  ...
  'semantic_successor':'V114',
  'semantic_correction':'REPRESENTED_DENOTATUM_RELATION_SUPPORT_ORIENTATION_COVERAGE'
...
python -m py_compile researcher_inventory/v114_task_rules.py researcher_inventory/v114_openai_agents_adapter.py researcher_inventory/v114_harness_calibration_runner.py researcher_inventory/v114_harness_holdout_runner.py
...
echo '=== V114 DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v114_harness_calibration_runner

echo '=== V114 REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v114_harness_calibration_runner

echo '=== V114 REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v114_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V114 represented-denotatum/relation/support/orientation coverage passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF

# First and only calibrated-lineage sealed holdout attempt, reachable only after all paired archetype batches pass.
python -m researcher_inventory.v114_harness_holdout_runner
...
# session_bootstrap always POSTs a brand-new saved agent. It receives only the finalized V114 durable contract.
python researcher_inventory/session_bootstrap.py
python -m researcher_inventory.v114_harness_holdout_runner

cat > researcher_inventory/status/CLEAN_ROOM_CERTIFIED.md <<'EOF'
# Researcher Inventory Apparatus Clean-Room Certification
V114 passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one finalized contract. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V114 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
