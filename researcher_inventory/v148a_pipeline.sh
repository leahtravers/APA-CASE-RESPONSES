...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v148_session_recovery || cp -R researcher_inventory/runtime/v148_session_recovery "$HISTORY_DIR/v148_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V148_CLASS_COMPLETE_SOURCE_ROLE_INVENTORY',
  'harness_successor':'V148A_ONE_TURN_CLASS_COMPLETE_SOURCE_ROLE_TOPOLOGY',
  'harness_corrections':'V66_FIELD_ALIGNMENT + V88A_EDITOR_SHORTHAND_ALIGNMENT + V92_SAME_SESSION_TRANSIENT_RECOVERY + V126A_CONTRACT_SUBORDINATE_REQUEST_ENVELOPE + V146A_ONE_TURN_TOPOLOGY + V148A_CLASS_LOCAL_SELECTOR'
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V148 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
echo '=== V148 CONTRACT / V148A CLASS-COMPLETE HARNESS DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v148a_harness_calibration_runner

echo '=== V148 CONTRACT / V148A HARNESS REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v148a_harness_calibration_runner

echo '=== V148 CONTRACT / V148A HARNESS REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v148a_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V148 durable class-complete source-role contract with the V148A whole-source class-local harness passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF

python -m researcher_inventory.v148a_harness_holdout_runner
...
The finalized V148 durable class-complete source-role contract, executed through the V148A whole-source class-local harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V148 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
