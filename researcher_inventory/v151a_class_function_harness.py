#!/usr/bin/env python3
"""V151A whole-source class-function-complete harness.

RI-CONTRACT-V151 is the sole durable semantic authority. This harness keeps the proven
single-request whole-source topology, deterministic normalization, frozen primitive
references, and compound normalization. It intentionally adds no semantic gold,
evaluator findings, expected counts, case-specific corrections, or holdout material.
"""
...
class V151AClassFunctionCompleteApparatus(v149a.V149AInventoryCoordinateApparatus):
    """Execute the installed V151 durable contract in one coherent whole-source request."""

    def _extract_integrated(self, source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        payload = {
            "task": "researcher_inventory_build_class_function_complete_inventory",
        ...
        raise apparatus.ApparatusError(f"integrated V151A extraction failed after retries: {last}")
    ...
    def run(self, source: str, source_case_ref: str | None = None, researcher_interest: str | None = None, created_by_ref: str = "RI-WORKER") -> dict[str, Any]:
        ...
        result["validation"]["notes"] = ["V151 durable source-role completeness contract executed through one-turn whole-source mechanical topology plus deterministic validation"]
        return result
