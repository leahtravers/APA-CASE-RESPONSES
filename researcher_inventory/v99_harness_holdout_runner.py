#!/usr/bin/env python3
"""V99 sealed-holdout entry point retaining proven evaluator/isolation controls.

Workflow invocation remains gated on repeated Case 2 + Case 6 passes under one
finalized V99 contract and saved-agent lineage. This module never exposes sealed
...
apparatus.BASE_RULES = V99_BASE_RULES
apparatus.CLASS_RULES = V99_CLASS_RULES
base.InventoryApparatus = V66HarnessInventoryApparatus

_OriginalCommandAdapter = base.CommandAdapter

class V99HarnessCommandAdapter(_OriginalCommandAdapter):
    def __init__(self, command: str, timeout: int = 2400):
        super().__init__(command, timeout=timeout)

base.CommandAdapter = V99HarnessCommandAdapter
