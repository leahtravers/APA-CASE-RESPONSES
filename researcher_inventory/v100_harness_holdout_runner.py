#!/usr/bin/env python3
"""V100 sealed-holdout entry point retaining proven evaluator/isolation controls.

Workflow invocation remains gated on repeated Case 2 + Case 6 passes under one
finalized V100 contract and saved-agent lineage. This module never exposes sealed
...
apparatus.BASE_RULES = V100_BASE_RULES
apparatus.CLASS_RULES = V100_CLASS_RULES
base.InventoryApparatus = V66HarnessInventoryApparatus

_OriginalCommandAdapter = base.CommandAdapter

class V100HarnessCommandAdapter(_OriginalCommandAdapter):
    def __init__(self, command: str, timeout: int = 2400):
        super().__init__(command, timeout=timeout)

base.CommandAdapter = V100HarnessCommandAdapter
