...
class V147AHybridHarnessTests(unittest.TestCase):
    def test_whole_source_hybrid_uses_one_request(self):
        adapter = CaptureAdapter(minimal_response())
        result = V147AHybridInventoryApparatus(adapter).run("I waited at the store.", "test-case", "lightweight researcher inventory only")
        self.assertEqual(len(adapter.payloads), 1)
        payload = adapter.payloads[0]
        self.assertEqual(payload["task"], "researcher_inventory_build_scaffold_bounded_inventory")
        self.assertIn("structural person/object/place/time scaffold", payload["rules"].lower())
        self.assertIn("scaffold-bounded", payload["rules"].lower())
    ...
    def test_unknown_compound_member_fails(self):
        ...
        with self.assertRaises(apparatus.ApparatusError):
            V147AHybridInventoryApparatus(CaptureAdapter(response), retries=0).run("I waited at the store.")

    def test_runtime_rule_is_gold_blind_and_contract_subordinate(self):
        lower = V147A_INTEGRATED_RULE.lower()
        self.assertIn("durable contract", lower)
        self.assertIn("scaffold", lower)
        self.assertIn("relation ceiling", lower)
        self.assertIn("sealed-holdout material", lower)
        self.assertNotIn("d47915552fdbdd23adc8c3f66ab06f94e022ab0bfd93faa5faabb53fdf9c9193", V147A_INTEGRATED_RULE)
        self.assertNotIn("50d646aad08b33051b93c1f3b3538d59ff78386558473b71a906a94213635070", V147A_INTEGRATED_RULE)
