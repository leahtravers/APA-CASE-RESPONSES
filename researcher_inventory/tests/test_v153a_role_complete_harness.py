...
class V153ARoleCompleteHarnessTests(unittest.TestCase):
    def test_whole_source_inventory_uses_one_neutral_request(self):
        adapter = CaptureAdapter(minimal_response())
        result = V153ARoleCompleteInventoryApparatus(adapter).run("I waited at the store.", "test-case", "lightweight researcher inventory only")
        self.assertEqual(len(adapter.payloads), 1)
        payload = adapter.payloads[0]
        self.assertEqual(payload["task"], "researcher_inventory_build")
        for forbidden in ("canonical", "primitive", "complete", "promoted", "gold", "archetype", "holdout"):
            self.assertNotIn(forbidden, payload["task"].lower())
    ...
    def test_unknown_compound_member_fails(self):
        ...
        with self.assertRaises(apparatus.ApparatusError):
            V153ARoleCompleteInventoryApparatus(CaptureAdapter(response), retries=0).run("I waited at the store.")

    def test_harness_contains_no_gold_or_holdout_material(self):
        import inspect
        from researcher_inventory import v153a_role_complete_harness as module
