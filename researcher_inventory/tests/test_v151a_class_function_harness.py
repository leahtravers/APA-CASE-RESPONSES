...
class V151AClassFunctionHarnessTests(unittest.TestCase):
    def test_whole_source_class_function_inventory_uses_one_request(self):
        adapter = CaptureAdapter(minimal_response())
        result = V151AClassFunctionCompleteApparatus(adapter).run("I waited at the store.", "test-case", "lightweight researcher inventory only")
        self.assertEqual(len(adapter.payloads), 1)
        payload = adapter.payloads[0]
        self.assertEqual(payload["task"], "researcher_inventory_build_class_function_complete_inventory")
    ...
    def test_unknown_compound_member_fails(self):
        ...
        with self.assertRaises(apparatus.ApparatusError):
            V151AClassFunctionCompleteApparatus(CaptureAdapter(response), retries=0).run("I waited at the store.")

    def test_harness_contains_no_gold_or_holdout_material(self):
        import inspect
        from researcher_inventory import v151a_class_function_harness as module
