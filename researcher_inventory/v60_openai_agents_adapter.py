#!/usr/bin/env python3
"""V60 contract-version-correct isolated agent adapter.

Preserves the V59 transport/session-recovery behavior unchanged in substance. V60 changes semantic instructions only. Gold/evaluator/holdout material is never sent to the worker.
"""
...
base.CONTRACT_VERSION = os.environ.get("CONTRACT_VERSION", "RI-CONTRACT-V60").strip() or "RI-CONTRACT-V60"
RECOVERY_DIR = Path("researcher_inventory/runtime/v60_session_recovery")
...
def _is_transient_transport_error(exc: Exception) -> bool:
    text = str(exc).lower()
    markers = (
        "http 500", "http 502", "http 503", "http 504", "internal error",
        "bad gateway", "service unavailable", "gateway timeout", "temporar",
        "connection reset", "connection refused", "remote end closed", "timed out",
    )
...
def _session_v60(agent_id: str, prompt: str, session_type: str, state_path: Path, trace_path: Path, phase: str):
    ...
    try:
        ...
        if str(exc) == "session timeout":
            try:
                ...
                _save_uncertain(state_path, trace_path, state, phase, phase_state, session_type, session_id, status_exc, "timeout_status_unknown")
                raise recovery.SessionUncertain(f"V60_SESSION_UNCERTAIN session_id={session_id} status=unknown") from exc
            ...
            if status == "idle":
                ...
                raise recovery.SessionUncertain(f"V60_SESSION_UNCERTAIN session_id={session_id} status={status}") from exc
        elif _is_transient_transport_error(exc):
            _save_uncertain(state_path, trace_path, state, phase, phase_state, session_type, session_id, exc, "transient_wait_uncertain")
            raise recovery.SessionUncertain(f"V60_SESSION_UNCERTAIN session_id={session_id} status=unknown transient_transport") from exc
        else:
    ...
    try:
        ...
        if _is_transient_transport_error(exc):
            ...
            _save_uncertain(state_path, trace_path, state, phase, phase_state, session_type, session_id, exc, "transient_result_read_uncertain")
            raise recovery.SessionUncertain(f"V60_SESSION_UNCERTAIN session_id={session_id} status=unknown transient_result_read") from exc
        raise
...
base.recovery._session = _session_v60
