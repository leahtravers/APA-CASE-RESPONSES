#!/usr/bin/env python3
"""V148A whole-source class-complete / class-local-exclusion harness.

RI-CONTRACT-V148 remains the sole durable semantic authority. This successor retains
the proven single-turn whole-source topology while changing only the operation selector
so it follows V148's positive class capture and class-local exclusion order. No
...
from researcher_inventory import v147a_hybrid_harness as v147a
...
class V148AClassCompleteInventoryApparatus(v147a.V147AHybridInventoryApparatus):
    """Execute V148 semantics in one coherent whole-source model turn."""

    def _extract_integrated(self, source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        payload = {
            "task": "researcher_inventory_build_class_complete_source_role_inventory",
            "rules": V126A_BASE_RULES + "\n" + V148A_INTEGRATED_RULE,
            "source": source,
            "response_schema": {
        ...
        raise apparatus.ApparatusError(f"integrated V148A extraction failed after retries: {last}")
    ...
    def run(self, source: str, source_case_ref: str | None = None, researcher_interest: str | None = None, created_by_ref: str = "RI-WORKER") -> dict[str, Any]:
        ...
        result["validation"]["notes"] = ["V148A one-turn whole-source class-complete / class-local-exclusion semantic pass plus mechanical validation passed"]
        return result
