#!/usr/bin/env python3
"""V152A whole-source canonical-primitive harness.

RI-CONTRACT-V152 is the sole durable semantic authority. This harness preserves the
proven one-request whole-source topology, deterministic normalization, frozen primitive
references, and compound normalization. It adds no semantic gold, evaluator findings,
expected counts, case-specific corrections, or holdout material.
"""
...
class V152ACanonicalPrimitiveApparatus(v149a.V149AInventoryCoordinateApparatus):
    """Execute the installed V152 durable contract in one coherent whole-source request."""

    def _extract_integrated(self, source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        payload = {
            "task": "researcher_inventory_build_canonical_primitive_inventory",
        ...
        raise apparatus.ApparatusError(f"integrated V152A extraction failed after retries: {last}")
    ...
    def run(self, source: str, source_case_ref: str | None = None, researcher_interest: str | None = None, created_by_ref: str = "RI-WORKER") -> dict[str, Any]:
        ...
        result["validation"]["notes"] = ["V152 durable canonical-primitive contract executed through one-turn whole-source mechanical topology plus deterministic validation"]
        return result
