#!/usr/bin/env python3
"""V39 harness-isolated adapter.

The durable V39 contract is the sole semantic selection/grain authority. The
...
recovery.RECOVERY_DIR = Path("researcher_inventory/runtime/v39_contract_isolated_session_recovery")
...
def _attention_for(bounded: dict) -> str:
    ...
    if correction:
        correction_note = (
            ...
            + correction
            + " Rebuild the complete requested result under the installed V39 contract; "
    ...
    if bounded.get("task") == "researcher_inventory_extract_one_class":
        return (
            "UNIT PASS. Apply the installed durable V39 contract exactly to the complete source "
            "for the requested class. Read the whole source, reconstruct represented source "
            "structure, then admit source-grounded units directly by positive class-native function "
            "and the contract's explicit exclusions/coreference rules. Do not apply a whole-binding "
            "non-substitutability prerequisite. Follow response_schema exactly. Do not infer an "
            "expected count or hidden archetype."
            + correction_note
        )
    if bounded.get("task") == "researcher_inventory_build_lightweight_compounds":
        return (
            "COMPOUND PASS. Apply the installed durable V39 contract exactly to the complete source "
            "and supplied frozen final units. Reconstruct source-presented operative bindings among "
            "the frozen units without creating, suppressing, or repairing units and without requiring "
            "global non-substitutability. Follow response_schema exactly. Do not infer an expected "
            "count or hidden archetype."
            + correction_note
        )
    return "Apply the installed durable V39 contract exactly to this bounded request." + correction_note
...
def main() -> int:
    ...
    if not bounded.get("correction") and state_path.exists():
        recovery._trace(
            ...
            "researcher_inventory_v39_contract_isolated",
            detail="prior local state preserved in trace; new request starts fresh",
        )
        state_path.unlink()

    initial_prompt = (
        "Execute only this bounded request under your installed durable V39 contract. "
    ...
    try:
        provisional = recovery._session(
            agent_id,
            initial_prompt,
            "researcher_inventory_v39_contract_isolated_extract",
            state_path,
            trace_path,
            "provisional",
        )
        ...
        verify_prompt = (
            "Perform a separate adjudication in a new session under the SAME installed durable V39 "
            "contract. Re-read the complete source and independently rebuild the requested result "
            "before comparing with the provisional output. Apply positive class-native unit admission "
            "with explicit exclusions/coreference, not whole-binding deletion. The V39 durable "
            "contract remains the sole semantic selection/grain authority. Return the complete "
        ...
        verified = recovery._session(
            agent_id,
            verify_prompt,
            "researcher_inventory_v39_contract_isolated_adjudication",
            state_path,
            trace_path,
            "verified",
        )
