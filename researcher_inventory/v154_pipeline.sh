...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v154_session_recovery || cp -R researcher_inventory/runtime/v154_session_recovery "$HISTORY_DIR/v154_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V154_STRUCTURAL_LATTICE_PLUS_MINIMAL_RELATION_NUCLEUS',
  'harness_successor':'V153A_NEUTRAL_ONE_TURN_WHOLE_SOURCE_TOPOLOGY_RETAINED',
  'harness_corrections':'V66_FIELD_ALIGNMENT + V88A_EDITOR_SHORTHAND_ALIGNMENT + V92_SAME_SESSION_TRANSIENT_RECOVERY + V126A_CONTRACT_SUBORDINATE_REQUEST_ENVELOPE + V146A_ONE_TURN_TOPOLOGY + V149A_COMPLETE_ATOM_NORMALIZATION + V153A_NEUTRAL_TASK_SELECTOR + OUTER_TIMEOUT_2400'
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V154 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
echo '=== V154 CONTRACT / RETAINED V153A NEUTRAL WHOLE-SOURCE HARNESS DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v153a_harness_calibration_runner

echo '=== V154 CONTRACT / RETAINED V153A HARNESS REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v153a_harness_calibration_runner

echo '=== V154 CONTRACT / RETAINED V153A HARNESS REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v153a_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V154 durable structural-lattice/minimal-relation-nucleus contract, executed through the retained V153A neutral one-turn whole-source harness, passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF

python -m researcher_inventory.v153a_harness_holdout_runner
...
The finalized V154 durable structural-lattice/minimal-relation-nucleus contract, executed through the retained V153A neutral one-turn whole-source harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V154 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
