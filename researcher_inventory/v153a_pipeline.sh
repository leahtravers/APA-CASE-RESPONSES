...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v153_session_recovery || cp -R researcher_inventory/runtime/v153_session_recovery "$HISTORY_DIR/v153_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V153_RESEARCHER_ADDRESSABLE_ROLE_AT_SOURCE_STAGED_GRAIN',
  'harness_successor':'V153A_NEUTRAL_ONE_TURN_ROLE_COMPLETE_TOPOLOGY',
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V153 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m py_compile researcher_inventory/v153a_role_complete_harness.py researcher_inventory/v153a_harness_calibration_runner.py researcher_inventory/v153a_harness_holdout_runner.py researcher_inventory/v153_openai_agents_adapter.py
...
The finalized V153 durable researcher-addressable role contract, executed through the V153A neutral one-turn whole-source harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V153 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
