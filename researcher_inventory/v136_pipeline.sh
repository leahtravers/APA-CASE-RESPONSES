...
preserve_history() {
  ...
  test ! -d researcher_inventory/runtime/v136_session_recovery || cp -R researcher_inventory/runtime/v136_session_recovery "$HISTORY_DIR/v136_session_recovery"
...
print(json.dumps({
  ...
  'semantic_contract':'V136_SELECTABLE_SOURCE_COORDINATE_CLASS_SPECIFIC_WARRANT',
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "training-history: preserve researcher inventory V136 run ${GITHUB_RUN_ID} attempt ${GITHUB_RUN_ATTEMPT} [skip ci]"
...
python -m py_compile researcher_inventory/v126a_task_rules.py researcher_inventory/v126a_harness_prompt_correction.py researcher_inventory/v126a_harness_calibration_runner.py researcher_inventory/v126a_harness_holdout_runner.py researcher_inventory/v136_openai_agents_adapter.py
...
echo '=== V136 CONTRACT / V126A CONTRACT-SUBORDINATE HARNESS DIAGNOSTIC ARCHETYPE BATCH ==='
TEST_REPETITIONS=1 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V136 CONTRACT / V126A HARNESS REPEATABILITY BATCH 1 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

echo '=== V136 CONTRACT / V126A HARNESS REPEATABILITY BATCH 2 ==='
TEST_REPETITIONS=2 python -m researcher_inventory.v126a_harness_calibration_runner

cat > researcher_inventory/status/ARCHETYPE_CALIBRATED.md <<'EOF'
# Researcher Inventory Apparatus Archetype Calibration
V136 durable contract with the retained V126A contract-subordinate runtime prompt harness passed the immutable Case 2/Case 6 SHA-256 gate, one diagnostic batch, and two repeated verification batches under the same finalized durable contract and saved-agent lineage. The worker had no archetype access.
EOF
...
The finalized V136 durable contract, executed through the retained V126A contract-subordinate runtime prompt harness, passed immutable workbook verification and repeated hidden Case 2/Case 6 calibration under one saved-agent lineage. The calibrated lineage passed the sealed holdout exactly once and was retired. A brand-new saved agent bootstrapped only from finalized V136 durable instructions, with no prior session or holdout output, then passed the sealed holdout in a new session.
EOF
