#!/usr/bin/env python3
"""V96 sealed-holdout entry point retaining proven evaluator/isolation controls.

Workflow invocation remains gated on repeated Case 2 + Case 6 passes under one
finalized V96 contract and saved-agent lineage. This module never exposes sealed
...
apparatus.BASE_RULES = V96_BASE_RULES
apparatus.CLASS_RULES = V96_CLASS_RULES
base.InventoryApparatus = V66HarnessInventoryApparatus

_OriginalCommandAdapter = base.CommandAdapter

class V96HarnessCommandAdapter(_OriginalCommandAdapter):
    def __init__(self, command: str, timeout: int = 2400):
        super().__init__(command, timeout=timeout)

base.CommandAdapter = V96HarnessCommandAdapter
